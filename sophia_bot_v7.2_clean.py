...
def get_acquisition_breakdown(users=None):
    ...
    try:
        ...
        for uid in users:
            ...
            if user_has_paid(uid):
                row["paid"] += 1
                row["estimatedRevenue"] += float(PRECO_VIP.replace("R$", "").replace(",", ".").strip() or 9.9)
...
def get_realistic_start_message(uid, ia_config=None):
    """Abertura curta do /start, feita para gerar uma ação simples em vez de texto livre."""
    user_name = get_user_name(uid)
    if user_name:
        return f"Chegou, {user_name} 😏 Quer ver uma prévia primeiro ou quer ver como funciona o VIP?"
    return "Chegou 😏 Quer ver uma prévia primeiro ou quer ver como funciona o VIP?"
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Novo onboarding: resposta rápida + dois botões. O lead não precisa digitar
        # para mostrar intenção, e nenhuma mídia é entregue automaticamente no /start.
        ...
        try:
            ...
            fallback = "Chegou 😏 Quer ver uma prévia primeiro ou ver o acesso VIP?"
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=fallback,
                reply_markup=keyboard,
        ...
        # IMPORTANTE: não enviar FOTO_APOS_START/VIDEO_APOS_START aqui.
        # A primeira mídia só sai quando o próprio lead toca em 👀 VER PRÉVIA.
...
def admin_stats():
    ...
    try:
        ...
        by_source = {}
        try:
            vip_price = float(PRECO_VIP.replace("R$", "").replace(",", ".").strip() or 9.9)
        except Exception:
            vip_price = 9.9
